...
def build_visualization(program_file_path, profiler_results, e2e_results: Dict[int, InputSizeResult]) -> dict:
    ...
    for i in e2e_results:
        ...
        e2e_memory.append({
            "n": i,
            "total_memory": round(e2e_results[i].average.max_memory_usage_bytes / 10**6, 2),
            "O(1)": round(fit_data_memory.O_1[i] / 10**6, 2),
            "O(log(n))": round(fit_data_memory.O_logn[i] / 10**6, 2),
            "O(n)": round(fit_data_memory.O_n[i] / 10**6, 2),
            "O(n\u00B2)": round(fit_data_memory.O_n2[i] / 10**6, 2),
            "O(n\u00B3)": round(fit_data_memory.O_n3[i] / 10**6, 2),
            "O(nlog(n))": round(fit_data_memory.O_nlogn[i] / 10**6, 2),
            "O(n\u207F)": round(fit_data_memory.O_nn[i] / 10**6, 2),
